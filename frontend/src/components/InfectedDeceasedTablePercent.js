...
function InfectedDeceasedTablePercent({ eventData, currentIndex }) {
            ...
            {filteredData.length === 0 ? (
              ...
              filteredData.map((county, index) => (
                <tr key={index} className={index % 2 === 0 ? 'even-row' : 'odd-row'}>
                  <td>{county.county}</td>
                  <td>{county.infectedPercent}</td>
                  <td>{county.deceasedPercent}</td>
                </tr>
              ))
            )}
...
export default InfectedDeceasedTablePercent;
