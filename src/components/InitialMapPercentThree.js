...
// Color mapping function with additional intervals
const getColor = (infectedPercent) => {
  return infectedPercent > 0.3 ? '#800026' :
         infectedPercent > 0.25 ? '#BD0026' :
         infectedPercent > 0.2 ? '#E31A1C' :
         infectedPercent > 0.15 ? '#FC4E2A' :
...
// Legend component integrated into InitialMapPercentThree
const InitialMapPercentThree = ({ outputData }) => {
  ...
  const Legend = () => {
    ...
    useEffect(() => {
      ...
      legend.onAdd = function () {
        ...
        const grades = [0.3, 0.25, 0.2, 0.15, 0.1, 0.05, 0];
        const labels = [];
...
export default InitialMapPercentThree;
